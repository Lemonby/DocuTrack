    ...
    <section class="bg-white p-4 md:p-7 rounded-2xl shadow-lg overflow-hidden mb-6 flex flex-col">
        <div class="flex flex-col md:flex-row md:justify-between md:items-center mb-6 pb-5 border-b border-gray-200 gap-3">
            <div class="flex-shrink-0">
                <h2 class="text-xl md:text-2xl font-bold text-gray-800">Antrian Validasi LPJ</h2>
                <p class="text-sm text-gray-500 mt-1 hidden md:block">Laporan Pertanggungjawaban yang perlu diverifikasi oleh Bendahara.</p>
            </div>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function render() {
        ...
        const empty = `<tr><td colspan="6" class="px-6 py-14 text-center"><i class="fas fa-inbox text-3xl text-gray-300 block mb-2"></i><span class="text-gray-400 text-sm">${allData.length===0?'Belum ada LPJ.':'Data tidak ditemukan.'}</span></td></tr>`;
        ...
        if (tbody) tbody.innerHTML = pageData.length===0 ? empty : pageData.map((item,idx) => {
            ...
            return `<tr class="hover:bg-gray-50 transition-colors">
                ...
                <td class="px-6 py-5 text-sm">
                    ...
                    <div class="text-xs text-gray-400"><i class="fas fa-graduation-cap mr-1 text-emerald-400"></i>${esc(item.jurusan||'-')}</div>
                </td>
                <td class="px-6 py-5 text-sm text-gray-600 whitespace-nowrap"><i class="fas fa-calendar-alt text-gray-400 mr-1"></i>${fmt(item.tanggal_pengajuan)}</td>
                <td class="px-6 py-5 text-sm text-gray-600 whitespace-nowrap">${item.deadline?`<i class="fas fa-clock text-emerald-500 mr-1"></i>${fmt(item.deadline)}`:'<span class="text-gray-400">-</span>'}</td>
                <td class="px-6 py-5">${badge(item.status)}</td>
                <td class="px-6 py-5">
                    <a href="/bendahara/lpj/show/${item.id||0}" class="inline-flex items-center gap-1.5 bg-emerald-600 text-white px-4 py-2 rounded-lg text-xs font-semibold hover:bg-emerald-700 transition-colors shadow-sm">
                        <i class="fas fa-file-alt"></i> Validasi
                    </a>
                </td>
            </tr>`;
        }).join('');
        ...
        if (mobileList) mobileList.innerHTML = pageData.length===0
            ...
            : pageData.map((item,idx) => {
                ...
                return `<div class="bg-white border border-gray-200 border-l-4 border-l-emerald-500 rounded-xl p-4 shadow-sm">
                    <div class="flex justify-between items-center mb-3 pb-3 border-b border-gray-100"><span class="bg-emerald-600 text-white text-xs font-bold px-2.5 py-1 rounded-lg">#${no}</span>${badge(item.status)}</div>
                    ...
                    <a href="/bendahara/lpj/show/${item.id||0}" class="w-full flex items-center justify-center gap-2 bg-emerald-600 text-white py-2.5 rounded-lg text-sm font-semibold hover:bg-emerald-700 transition-colors">
                        <i class="fas fa-file-alt"></i> Validasi LPJ
                    </a>
                </div>`;
            }).join('');
