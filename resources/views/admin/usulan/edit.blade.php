...
<main class="main-content font-poppins p-4 md:p-7 -mt-8 md:-mt-20 max-w-7xl mx-auto w-full">
    ...
    <section id="form-section">
        <form id="kak-form-element" action="{{ url('/admin/pengajuan-usulan/update', $id) }}" method="POST" enctype="multipart/form-data">
                ...
                <nav aria-label="Progress">
                    <ol class="flex items-center justify-between gap-2">
                        ...
                        <li class="flex flex-col items-center relative flex-1">
                            <div id="step-indicator-{{ $i+1 }}" class="w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold {{ $i === 0 ? 'bg-amber-600 text-white ring-4 ring-amber-100' : 'bg-gray-200 text-gray-500' }} transition-all duration-300">
                                <span>{{ $i+1 }}</span>
                            </div>
                            <div class="mt-2 text-center">
                                <span class="text-xs font-semibold {{ $i === 0 ? 'text-amber-700' : 'text-gray-400' }}">{{ $step }}</span>
                            </div>
                            @if($i < 3)
            ...
            {{-- Catatan Revisi Alert (Always Visible) --}}
            @if(isset($catatan_revisi) || (isset($status) && $status === 'Revisi'))
            <div class="mb-8 bg-amber-50 border-l-4 border-amber-500 p-4 rounded-r-lg shadow-sm">
                <div class="flex items-start">
                    <i class="fas fa-exclamation-triangle text-amber-500 mt-1 mr-3"></i>
                    <div>
                        <h4 class="text-sm font-bold text-amber-800 uppercase tracking-widest mb-1">Catatan Revisi Verifikator</h4>
                        <p class="text-sm text-amber-700">{{ $catatan_revisi ?? 'Rincian RAB untuk Honor Pemateri terlalu tinggi, mohon sesuaikan dengan standar SBM terbaru. Gambaran umum juga perlu diperjelas terkait output kegiatannya.' }}</p>
                    </div>
                </div>
            </div>
            @endif
            ...
            <div class="form-content-wrapper relative min-h-[500px]">
                ...
                <div id="form-tahap-1" class="form-step active">
                    <section class="bg-white p-4 md:p-8 rounded-2xl shadow-lg overflow-hidden">
                        <div class="mb-8">
                            <h2 class="text-xl md:text-2xl font-bold text-gray-800 mb-6 text-center">Revisi Usulan Kegiatan #USL-{{ str_pad($id, 5, '0', STR_PAD_LEFT) }}</h2>
                                ...
                                <div class="relative">
                                    ...
                                    <div class="relative">
                                        ...
                                        <select id="jurusan" name="jurusan" required class="block w-full px-4 py-3.5 pr-10 text-sm text-gray-800 bg-white rounded-lg border border-gray-300 appearance-none focus:outline-none focus:ring-2 focus:border-blue-600 focus:ring-blue-600 cursor-pointer">
                                            ...
                                            <option value="Teknik Elektro" {{ $jrs == 'Teknik Elektro' ? 'selected' : '' }}>Teknik Elektro</option>
                                            <option value="Teknik Informatika dan Komputer" {{ $jrs == 'Teknik Informatika dan Komputer' ? 'selected' : '' }}>Teknik Informatika dan Komputer</option>
                                            <option value="Teknik Grafika dan Penerbitan" {{ $jrs == 'Teknik Grafika dan Penerbitan' ? 'selected' : '' }}>Teknik Grafika dan Penerbitan</option>
                                            <option value="Akuntansi" {{ $jrs == 'Akuntansi' ? 'selected' : '' }}>Akuntansi</option>
                                            <option value="Administrasi Niaga" {{ $jrs == 'Administrasi Niaga' ? 'selected' : '' }}>Administrasi Niaga</option>
                                            <option value="Pascasarjana" {{ $jrs == 'Pascasarjana' ? 'selected' : '' }}>Pascasarjana</option>
                                        </select>
                                ...
                                <div class="md:col-span-2 relative">
                                    ...
                                    <div class="relative">
                                        ...
                                        <select id="wadir_tujuan" name="wadir_tujuan" required class="block w-full px-4 py-3.5 pr-10 text-sm text-gray-800 bg-white rounded-lg border border-gray-300 appearance-none focus:outline-none focus:ring-2 focus:border-blue-600 focus:ring-blue-600 cursor-pointer">
                                            ...
                                            <option value="3" {{ str_contains($wadir, '3') || str_contains($wadir, 'Kemahasiswaan') ? 'selected' : '' }}>Wadir 3 - Kemahasiswaan</option>
                                            <option value="4" {{ str_contains($wadir, '4') ? 'selected' : '' }}>Wadir 4 - Kerjasama & Hubungan Luar</option>
                                        </select>
                                        <div class="absolute inset-y-0 right-0 flex items-center pr-3 pointer-events-none">
                                            <i class="fas fa-chevron-down text-gray-400"></i>
                                        </div>
...
<script>
    ...
    window.initialBudgetData = {!! isset($rab_data) ? json_encode($rab_data) : 'null' !!};
</script>
