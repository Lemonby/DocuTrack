...
@section('title', 'Persetujuan Kegiatan (PPK)')

@section('content')
<main class="main-content font-poppins p-3 sm:p-4 md:p-7 -mt-8 md:-mt-20 max-w-7xl mx-auto w-full">

    <section class="bg-white rounded-2xl shadow-sm overflow-hidden mb-8 border border-slate-100 flex flex-col">
        {{-- Header --}}
        <div class="p-5 border-b border-slate-50 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 bg-slate-50/50">
            <div>
                <h3 class="text-sm font-black text-slate-800 uppercase tracking-widest flex items-center gap-2">
                    <i class="fas fa-clipboard-check text-blue-600"></i> Persetujuan Usulan
                </h3>
                <p class="text-[10px] text-slate-400 font-bold uppercase tracking-tight mt-1 italic">Daftar kegiatan yang memerlukan persetujuan PPK</p>
            </div>
            <div class="flex flex-col sm:flex-row gap-2">
                ...
                <select id="filter-jurusan" class="px-4 py-2 bg-white border border-slate-200 rounded-xl text-[10px] font-black uppercase tracking-widest text-slate-600 outline-none focus:ring-2 focus:ring-blue-500 appearance-none cursor-pointer">
                    <option value="">Jurusan</option>
                    @foreach($jurusan_list as $j)
                    <option value="{{ $j }}">{{ $j }}</option>
                    @endforeach
                </select>
                <button id="reset-filter" class="p-2 bg-white border border-slate-200 text-slate-400 hover:text-blue-600 rounded-xl transition-all">
        ...
        {{-- Mobile List --}}
        <div class="md:hidden">
            <div id="mobile-list" class="p-4 space-y-4"></div>
        </div>
...
@push('scripts')
<script>
window.dataUsulan = @json($list_kegiatan ?? []);
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function render() {
        ...
        const empty = `<tr><td colspan="5" class="py-14 text-center text-[10px] font-black text-slate-300 uppercase tracking-widest">Data tidak ditemukan</td></tr>`;
        ...
        $id('table-body').innerHTML = data.length===0 ? empty : data.map((it,idx) => `
            <tr class="hover:bg-slate-50 transition-colors">
                ...
                <td class="px-6 py-5 text-sm">
                    ...
                    <div class="text-[9px] font-bold text-slate-400 mt-1 uppercase tracking-widest">${esc(it.pengusul||'')} (${esc(it.nim||'-')}) &bull; ${esc(it.prodi||'-')}</div>
                </td>
                <td class="px-6 py-5 text-[10px] font-bold text-slate-500 uppercase whitespace-nowrap">
                    <i class="far fa-calendar-alt text-blue-400 mr-2"></i>${fmt(it.tanggal_pengajuan)}
                </td>
                ...
                <td class="px-6 py-5 text-center">
                    <a href="/ppk/kegiatan/show/${it.id||0}" class="inline-flex items-center gap-2 bg-blue-600 text-white px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-blue-700 transition-all shadow-sm active:scale-95">
                        <i class="fas fa-search-plus text-[8px]"></i> Periksa
                    </a>
                </td>
            </tr>`).join('');

        $id('mobile-list').innerHTML = data.length===0 ?
            `<div class="text-center py-12 text-slate-300 text-[10px] font-black uppercase tracking-widest">Belum ada data</div>` :
            data.map((it,idx) => `
            <div class="bg-white border border-slate-200 rounded-xl p-4 shadow-sm group active:scale-[0.98] transition-all">
                <div class="flex justify-between items-center mb-3">
                    <span class="bg-slate-100 text-slate-500 text-[9px] font-black px-2 py-0.5 rounded uppercase tracking-widest">#${start+idx+1}</span>
                    ${badge(it.status)}
                </div>
                <p class="font-black text-slate-800 text-xs uppercase tracking-tight mb-1 group-hover:text-blue-600 transition-colors">${esc(it.nama||'Tanpa Judul')}</p>
                <p class="text-[9px] font-bold text-slate-400 uppercase tracking-widest italic mb-4">${esc(it.pengusul||'')} &bull; ${esc(it.prodi||'-')}</p>
                <a href="/ppk/kegiatan/show/${it.id||0}" class="w-full flex items-center justify-center gap-2 bg-slate-900 text-white py-3 rounded-xl text-[9px] font-black uppercase tracking-widest hover:bg-blue-600 transition-all">
                    <i class="fas fa-search-plus"></i> Periksa Kegiatan
                </a>
            </div>`).join('');
    ...
    $id('reset-filter')?.addEventListener('click', () => { $id('search-input').value=''; $id('filter-status').value=''; $id('filter-jurusan').value=''; apply(); });
