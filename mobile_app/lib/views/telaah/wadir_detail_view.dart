...
class WadirDetailView extends StatefulWidget {
  final int kegiatanId;

  const WadirDetailView({super.key, required this.kegiatanId});

  @override
  State<WadirDetailView> createState() => _WadirDetailViewState();
}

class _WadirDetailViewState extends State<WadirDetailView> {
  Kegiatan? _kegiatan;
  ...
  Future<void> _loadDetail() async {
    ...
    final detail = await provider.getTelaahDetail('wadir', widget.kegiatanId);
    if (mounted) {
      setState(() {
        if (detail != null) {
          _kegiatan = detail;
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Detail Usulan Kegiatan (Wadir)', style: TextStyle(fontWeight: FontWeight.w900, fontSize: 16, color: AppTheme.textDark)),
