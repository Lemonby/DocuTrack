...
class _AdminKegiatanDetailViewState extends State<AdminKegiatanDetailView> {
  ...
  void initState() {
    super.initState();
    _pjController = TextEditingController(text: widget.kegiatan.rawData?['penanggung_jawab']);
    _nimPjController = TextEditingController(text: widget.kegiatan.rawData?['nim_nip_pj']);
    
    _tglMulaiController = TextEditingController(text: widget.kegiatan.tanggalMulai);
    _tglSelesaiController = TextEditingController(text: widget.kegiatan.tanggalSelesai);

    if (widget.kegiatan.tanggalMulai != null) {
      _startDate = DateTime.tryParse(widget.kegiatan.tanggalMulai!);
    }
    if (widget.kegiatan.tanggalSelesai != null) {
      _endDate = DateTime.tryParse(widget.kegiatan.tanggalSelesai!);
    }
  }
    ...
    if (mounted) {
      if (success) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Rincian berhasil disubmit'), backgroundColor: Colors.green),
        );
      ...
      appBar: AppBar(
        title: Column(
          ...
          children: [
            ...
            Text('ID: #${widget.kegiatan.id}', style: const TextStyle(fontSize: 10, color: Colors.blue)),
          ],
        ),
            ...
            children: [
              ...
              Container(
                ...
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    _buildStatusHeader(statusColor, statusIcon, status),
                    ...
                    Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          _buildReadOnlyField('Nama Kegiatan', widget.kegiatan.namaKegiatan),
                          const SizedBox(height: 12),
                          _buildReadOnlyField('Pengusul', widget.kegiatan.pemilikKegiatan ?? '-'),
                          ...
                          if (!isReadonly) ...[
                            ...
                            const SizedBox(height: 16),
                            _buildInputField('NIM / NIP PJ *', _nimPjController, 'Masukkan NIM atau NIP', (v) => v!.isEmpty ? 'Wajib diisi' : null),
                          ] else ...[
                            _buildReadOnlyField('Nama Lengkap PJ', widget.kegiatan.rawData?['penanggung_jawab'] ?? '-'),
                            const SizedBox(height: 12),
                            _buildReadOnlyField('NIM / NIP PJ', widget.kegiatan.rawData?['nim_nip_pj'] ?? '-'),
                          ],
                          ...
                          if (!isReadonly) ...[
                            ...
                            Row(
                              children: [
                                Expanded(child: _buildReadOnlyField('Mulai', widget.kegiatan.tanggalMulai ?? '-', icon: Icons.event)),
                                ...
                                Expanded(child: _buildReadOnlyField('Selesai', widget.kegiatan.tanggalSelesai ?? '-', icon: Icons.event)),
                              ],
                            ),
                          ],
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text(widget.kegiatan.rawData?['revisi_comment'] ?? 'Mohon periksa kembali data penanggung jawab atau dokumen pendukung.', style: TextStyle(fontSize: 12, color: Colors.orange.shade800)),
              ],
            ),
          ),
        ],
  ...
  Widget _buildStatusHeader(Color statusColor, IconData statusIcon, String status) {
    return Container(
      padding: const EdgeInsets.all(20),
      decoration: BoxDecoration(
      ...
      child: Row(
        children: [
          ...
          const Spacer(),
          Text(widget.kegiatan.jurusanPenyelenggara ?? '-', style: const TextStyle(color: AppTheme.textMuted, fontSize: 10, fontWeight: FontWeight.bold)),
        ],
      ),
    );
  }
          ...
          child: Container(
            ...
            child: Column(
              children: [
                ...
                const SizedBox(height: 4),
                const Text('Format: PDF, DOC (Maks. 5MB)', style: TextStyle(fontSize: 11, color: Colors.blueGrey)),
              ],
            ),
          ),
        ),
  ...
  Widget _buildFileDisplay() {
    final fileName = widget.kegiatan.rawData?['surat_pengantar']?.toString().split('/').last ?? 'TIDAK_ADA_FILE.PDF';
    return Container(
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
